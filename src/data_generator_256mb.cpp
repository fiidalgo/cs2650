...
// Target size of 256MB in bytes
constexpr size_t TARGET_SIZE = 256ULL * 1024 * 1024;
...
{
    // Default output file
    std::string output_file = "data/test_data_256mb.bin";
    ...
    std::cout << "Generating 256MB of test data to " << output_file << std::endl;

    // Calculate number of pairs
    size_t num_pairs = NUM_PAIRS;
    size_t file_size = num_pairs * KV_PAIR_SIZE;
    double file_size_mb = static_cast<double>(file_size) / (1024 * 1024);

    std::cout << "Planning to generate " << num_pairs << " key-value pairs" << std::endl;
    std::cout << "Expected file size: " << file_size_mb << " MB" << std::endl;
    ...
    for (size_t batch = 0; batch < num_batches; ++batch)
    {
        ...
        if (batch % (num_batches / 4) == 0 || batch == num_batches - 1)
        {
            ...
            double data_size_mb = static_cast<double>(total_pairs_written * KV_PAIR_SIZE) /
                                  (1024 * 1024);
    ...
    double total_mb = static_cast<double>(total_pairs_written * KV_PAIR_SIZE) / (1024 * 1024);

    std::cout << "Data generation complete!" << std::endl;
    std::cout << "Total data generated: " << total_mb << " MB" << std::endl;
    std::cout << "Total pairs generated: " << total_pairs_written << std::endl;
    std::cout << "Time taken: " << total_time << " seconds" << std::endl;

    std::cout << "\nTo load this file into the LSM-Tree:" << std::endl;
    std::cout << "1. Start the server with 'make run-server'" << std::endl;
    std::cout << "2. Start the client with 'make run-client'" << std::endl;
    std::cout << "3. In the client, use command: l \"" << output_file << "\"" << std::endl;
